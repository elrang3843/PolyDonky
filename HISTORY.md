...
### Fixed
- **Fixed** — **단락 내 부분 텍스트 선택을 복사하면 단락 전체 + 가짜 줄바꿈이 붙여넣어지던 버그**. `TryCopyFlowSelection` 이 선택 영역과 무관하게 항상 Block 단위로 복사해, 부분 선택이라도 단락 전체가 직렬화되고 paste 시 새 Paragraph 가 anchor 뒤에 추가되어 단락 구분 줄바꿈이 추가로 보였음. `SelectionHasSpecialObject` 검사를 추가 — 선택 영역에 표·이미지·도형(BlockUIContainer/Tag-기반/Floater/InlineUIContainer)이 포함된 경우에만 PolyDonky.FlowSelection.v1 커스텀 경로를 사용하고, 순수 텍스트 선택은 WPF 기본 복사에 양보. 단락 부분 선택은 정확히 선택된 글자만 복사됨.
