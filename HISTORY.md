...
### Fixed
- **Fixed** — RTL 모드에서 텍스트 시작 위치가 오른쪽 여백 안쪽이 아닌 용지 오른쪽 경계선에 붙던 버그 (본문 편집기·글상자 모두). 원인: `BodyEditor.Padding.Right` 은 ContentHost 위치만 이동할 뿐, RTL paragraph 의 우측 정렬 기준점은 `FlowDocument.PageWidth` 끝(= 용지 경계)이 돼서 여백이 무시됐다. 글상자 `InnerEditor` 는 `FlowDocument.FlowDirection` 이 기본값 LTR 로 남아 있어 paragraph 정렬 자체가 LTR 이었다. 수정: `FlowDocumentBuilder` 에서 `PagePadding = new Thickness(padL, padT, padR, padB)` 로 여백을 FlowDocument 내부에서 확보. `BodyEditor.Padding = 0` 으로 이중 계산 방지. `InnerEditor` 도 컨테이너는 LTR 고정, `FlowDocument.FlowDirection = RightToLeft` 로 RTL 단락 정렬 처리. RTL 감지 조건을 `InnerEditor.FlowDirection` 에서 `InnerEditor.Document.FlowDirection` 으로 교체.
