...
> ✅ **정식 릴리스 — 현재 `v1.2.1`. 안정화 및 기능 개선이 계속 진행됩니다.**
> IWPF, DOCX, HWPX, HTML, XML, RTF, DOC(Word 97-2003), HWP 변환이 동작하며 WPF 편집기가 실행됩니다.
> 변경추적·수식·주석·고급 표 등 3단계 기능을 편집 UI 까지 포함해 지원합니다.
> 진행 상황은 [Issues](../../issues) / [Releases](../../releases) / [`HISTORY.md`](HISTORY.md) 에서 확인하세요.

### 버전 정책

- **프로젝트는 이미 정식 릴리스 단계입니다** — `1.0.0`(2026-05-20) 이후 `1.0.1` · `1.1.0` · `1.2.0` 을 거쳐 현재 **`1.2.1`** 까지 릴리스되었습니다.
- 모든 버전은 일반 [Semantic Versioning](https://semver.org/lang/ko/) 규칙(`1.2.1` / `1.3.0` / `2.0.0` ...)을 따르며, 릴리스 컷은 메인테이너의 명시적 결정이 있을 때만 진행합니다.
- 외부 배포 전 검증이 필요한 경우에 한해 `X.Y.Z-test.<n>` (pre-release) 빌드를 먼저 컷할 수 있습니다.
...
3. **3단계 (진행 중)** — 변경추적·주석·수식 편집 UI ✅, 고급 표(셀·행·열 속성·세로 정렬·외곽선 종류) ✅, EMF/WMF 렌더링 ✅ / 특수 조판은 계속 보강 중
...
[Releases 페이지](../../releases)에서 최신 정식 릴리스를 다운로드할 수 있습니다.
...
외부 배포 전 검증용으로 컷되는 pre-release 빌드입니다.
배포되는 경우 [Releases 페이지](../../releases)에 `Pre-release` 로 표시되며,
실험·평가 목적 외의 운영 사용은 권장되지 않습니다.

### 최신 개발 버전을 직접 빌드하려면

릴리스에 아직 포함되지 않은 변경을 쓰려면 소스에서 직접 빌드할 수 있습니다 → [소스에서 빌드하기](#소스에서-빌드하기) 참고.
