...
src/
  ...
  PolyDonky.Codecs.Docx/      DOCX codec (DocumentFormat.OpenXml) — CLI(PolyDonky.Convert.Docx) 전용
  PolyDonky.Codecs.Hwpx/      HWPX codec (자체 구현, KS X 6101) — CLI(PolyDonky.Convert.Hwpx) 전용
  PolyDonky.Codecs.Hwp/       HWP codec stub — v1.0.0 이후 자체 파서 구현 예정
  PolyDonky.Codecs.Html/      HTML5 codec (AngleSharp) — CLI(PolyDonky.Convert.Html) 전용
  PolyDonky.Codecs.Xml/       XML / XHTML5 codec (Codecs.Html 위에 polyglot serializer) — CLI(PolyDonky.Convert.Xml) 전용
...
- **프로젝트는 이미 정식 릴리스 단계다** — `1.0.0`(2026-05-20) 이후 `1.0.1` · `1.1.0` · `1.2.0` 을 거쳐
  현재 **`1.2.1`** 까지 정식 릴리스됨. 모든 버전은 일반 SemVer (`1.2.2` / `1.3.0` / `2.0.0` ...) 를 따른다.
  - **PATCH** — 하위호환 버그 수정만. **MINOR** — 하위호환 기능 추가. **MAJOR** — IWPF 포맷·공개 API 의 하위호환을 깨는 변경.
- 사용자의 명시적 지시 없이 다음을 절대 수행하지 않는다.
  - `## [Unreleased]` 를 `## [X.Y.Z]` 정식 버전 헤더로 승격
  - `vX.Y.Z` 정식 릴리스 태그 생성·푸시
  - GitHub Release 생성
- 사용자 지시가 있을 때의 절차:
  1. `[Unreleased]` 내용을 `## [X.Y.Z] - YYYY-MM-DD` 헤더로 승격하고 비어 있는 `[Unreleased]` 를 다시 만든다.
     변경 내용(Added/Fixed/...)을 보고 SemVer 규칙으로 X.Y.Z 를 결정한다.
  2. `Directory.Build.props` 의 `Version`/`FileVersion`/`AssemblyVersion` 과
     `AboutWindow.xaml` 의 버전 placeholder 도 같은 번호로 갱신한다 (About 창은 런타임에 어셈블리 버전을 읽음).
  3. 커밋 후 `git tag vX.Y.Z` (또는 테스트 컷이면 `vX.Y.Z-test.N`) 생성.
  4. 사용자가 푸시·릴리스 노트 작성을 별도 지시하면 그때 진행.
