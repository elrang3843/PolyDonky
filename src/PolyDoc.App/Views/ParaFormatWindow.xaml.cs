...
/// <summary>
/// 문단 서식 다이얼로그. RichTextBox 선택 영역에 걸쳐 있는 모든 단락의 정렬·줄간격·여백·들여쓰기·개요 수준을
/// 읽어 표시하고, OK 시 일괄 적용한다. 단락 Tag(PolyDoc.Paragraph) 의 Style 도 함께 갱신해
/// 비-FlowDocument 속성(IndentMm, SpaceBeforePt 등) 까지 라운드트립 보존한다.
/// </summary>
public partial class ParaFormatWindow : Window
{
    ...
    private void LoadCurrentFormatting()
    {
        ...
        try
        {
            ...
            // 배분 정렬은 PolyDoc.Alignment.Distributed (Tag 에서 읽음)
            ...
            // 줄 간격 / 여백 / 들여쓰기 — Tag 의 PolyDoc 모델에서 읽음 (비-WPF 속성 포함)
            var style = (paragraph?.Tag as PolyDoc.Core.Paragraph)?.Style ?? new ParagraphStyle();
