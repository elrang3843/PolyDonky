...
public partial class ParaFormatWindow : Window
{
    ...
    private void LoadCurrentFormatting()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (paragraph?.Tag is PolyDonky.Core.Paragraph polyP && polyP.Style.Alignment == Alignment.Distributed)
                    RbAlignDistributed.IsChecked = true;
                style = (paragraph?.Tag as PolyDonky.Core.Paragraph)?.Style ?? new ParagraphStyle();
            }
    ...
    private void ApplyToSelection()
    {
        ...
        foreach (var wpfPara in paragraphs)
        {
            ...
            // Tag(PolyDonky.Paragraph) 도 갱신 — 라운드트립 보존
            if (wpfPara.Tag is PolyDonky.Core.Paragraph polyPara)
            {
